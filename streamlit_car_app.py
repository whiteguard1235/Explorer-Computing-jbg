...
def cars_home():
    ...
    **대륙별 평균 연비, 마력(hp)과 연비(mpg) 관계, 차량 무게와 연비 관계, 연도별 평균 연비 변화, 실린더 수별 연비 변화**등을 시각화합니다.
    """)
    ...
    st.markdown("""
    1. **탐색적 자료분석 (EDA)**  
       - 제조 대륙별 평균 연비 비교, 마력과 차량 무게 대비 연비 분포, 연도별 연비 변화, 실린더 수별 연비 변화 등을 시각화합니다.  
...
def cars_EDA(df):
    ...
    **연비(mpg)와 주요 변수들 간의 관계, 대륙별 특성, 연도별 변화** 등을 탐색합니다.
    ...
    # 실린더 수별 평균 mpg 계산
    cylinder_avg = df.groupby("cylinders", as_index=False)["mpg"].mean()

    # 평균 점 + 선 추가
    fig5.add_scatter(
        x=cylinder_avg["cylinders"],
        y=cylinder_avg["mpg"],
        mode="markers+lines",
        name="실린더 수별 평균 연비",
        marker=dict(
    ...
    st.info("""
    💡 **분석 포인트**
    - 실린더수가 많아지면 연비가 낮아지는 경향이 있으나, 몇몇 구간에서 예외를 보여 유의한 분석이 아닐 가능성이 높습니다.      
    """)  
